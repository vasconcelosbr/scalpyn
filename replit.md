...
- **Hot hypertables: pre-apply DDL manually (invariant #8)**: hot tables (`ohlcv`, `indicators`, `decisions_log`, `trades`, `pipeline_watchlist_assets`) carry continuous writes from the previous-revision Celery + collector during deploy. `ALTER TABLE` / `CREATE INDEX` against them contests locks, hits `lock_timeout=10s` 3× in `start.sh`, and pushes the cold start past the 240 s startup probe. Apply the idempotent DDL (`ADD COLUMN IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS` — or `CREATE INDEX CONCURRENTLY` from `psql` for `ohlcv`) in Cloud SQL **before** pushing. Cold tables (`pools`, `pipeline_watchlists`, `users`, lookups) are safe via the normal Cloud Run path. Migrations 033 (`indicators.market_type`) and 034 (`ohlcv.market_type`) both tripped this on 2026-05-02; both fixed by manual DDL pre-apply.
...
- Manual recovery procedure for production drift: `docs/runbooks/scheduler-group-drift.md` — idempotent `ALTER TABLE … IF NOT EXISTS` + index, plus the post-fix Sentry / `/api/health/schema` / pool-budget validation checklist.
