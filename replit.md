...
- **Pipeline recovery 2026-05-08 (3 incidentes correlacionados)**: `_MICRO_GUARDS` time_limit 180→480s + `idle_in_transaction_session_timeout=300s`/`lock_timeout=30s` no Celery engine + `scalpyn` API ingress `internal-and-cloud-load-balancing`→`all`. Re-ler antes de mexer em `celery_app.py:_MICRO_GUARDS`, `database.py:_celery_connect_args`, `cloudbuild.yaml:--ingress`, ou de re-introduzir UPSERT em `market_metadata` no `_collect_5m_async`. Recovery manual (TX órfã segurando lock): `SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='scalpyn' AND state LIKE 'idle in transaction%' AND NOW()-state_change > INTERVAL '2 minutes';` (no Cloud Shell, com `--database=scalpyn` explícito — `$PGDATABASE` do DATABASE_URL vira `postgres` por padrão). Detalhes, sintomas, timelines e snippets em `backend/docs/runbooks/2026-05-08-pipeline-recovery.md`.
