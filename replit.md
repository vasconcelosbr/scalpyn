...
- **`TradeMonitorService` saída dinâmica via order flow (2026-05-12, dark rollout)**: extensão **estritamente aditiva** ao `_check_exit_conditions` (TP/SL/timeout sempre vencem). Hook em `trade_monitor_service.py` no ponto exato onde o loop fazia `continue` quando o check legado retornava None — só executa se `settings.TRADE_MONITOR_EXIT_FLOW_ENABLED=True` (default `False`, rollout dark). Lê `get_order_flow_data(symbol, window_seconds, market_type)` (module-level coroutine, NÃO classe injetada — import direto em `trade_monitor_service.py`) e dispara saída quando: (LONG) `taker_ratio < TRADE_MONITOR_EXIT_TAKER_BEAR_THRESHOLD` (default 0.35) OU dominância vendedora `> 1 - 1/TRADE_MONITOR_EXIT_VOLUME_SPIKE_THRESHOLD` (default spike=2.0 → dominance > 0.5); (SHORT) mirror simétrico via `1 - bear_threshold`. Stale guard: `data_age_seconds > TRADE_MONITOR_EXIT_FLOW_MAX_AGE_SECONDS` (default 20s) → no-op. Saída via `_close_trade(..., outcome=f"flow_{reason}")` com **tags curtas** (`flow_tb`=taker bear/LONG, `flow_ve`=volume exhaustion/LONG, `flow_tu`=taker bull/SHORT, `flow_vs`=volume squeeze/SHORT) — **CRÍTICO**: `trade_tracking.outcome` e `decisions_log.outcome` são `String(20)`, então outcome final tem que caber (todas as 4 tags geram `flow_xx` = 7 chars). NÃO usar reasons longos com valores numéricos no outcome (ex: `flow_taker_bear_0.312` = 21 chars QUEBRA persistência). Diagnóstico numérico (ratio, dominance) vai pro INFO log no `_check_exit_signals`, não pro outcome. Vocabulário estende `tp`/`sl`/`timeout` sem quebrar consumidores (passthrough). `summary["closed_flow"]` separado, somado ao total reportado em `tasks/trade_monitor.py`. **Por que NÃO usa `ConfigService`**: `ConfigService.get_config(db, config_type, user_id, pool_id)` exige `user_id` (NOT NULL UUID) e `TradeTracking` não tem `user_id`/`pool_id` — migration estaria fora do escopo aditivo. Thresholds via `settings`/env mantém Zero Hardcode (mesmo padrão do `TRADE_MONITOR_TIMEOUT_SECONDS` já presente no mesmo serviço). **Para ligar em prod**: setar `TRADE_MONITOR_EXIT_FLOW_ENABLED=1` no env do worker-execution apenas — outras réplicas não rodam essa task. Falha em `_check_exit_signals` nunca propaga (try/except + warning) — TP/SL/timeout permanecem invioláveis mesmo com order_flow_service quebrado/Redis indisponível.

- **`compute_structural_5m` — fecha gap 30m→5m do leitor estrutural (2026-05-12)**: `compute_30m` (crontab 0,30) escreve `scheduler_group='structural'` com idade p99 ≈ 1770s; `compute_5m` micro já calcula rsi/adx/macd/bollinger sobre candles 5m mas taga `'microstructure'`, então `indicators_provider.get_merged_indicators` (filtro por grupo) não enxerga. Nova task `app.tasks.compute_indicators.compute_structural_5m` (`_compute_structural_5m_async`, `compute_indicators.py:887+`) reusa candles 5m + `FeatureEngine.calculate(df, market_data=None, group="structural")` (filtro via `indicator_classifier.STRUCTURAL_CALC_KEYS` — ZERO HARDCODE de chaves) e grava em `indicators` com `timeframe='5m', scheduler_group='structural'`. Routing: `QUEUE_STRUCTURAL` (não competir com worker-micro). Guards: `_STRUCTURAL_GUARDS + _NO_REQUEUE_ON_WORKER_LOSS` (idempotente, chain-driven). Encadeada do `collect_5m` em paralelo ao `compute_5m` micro com `dedup_key="compute_structural_5m"` distinto. **CRÍTICO — `mode="insert_only"`** (queue) + `ON CONFLICT (time, symbol, timeframe) DO NOTHING` (direct INSERT): a UNIQUE real em `indicators` é `(time, symbol, timeframe)` SEM `scheduler_group`. Como compute_5m micro escreve `time=now()` no mesmo `(symbol, '5m')`, microsegundos diferentes geram linhas separadas no índice `(symbol, scheduler_group, time DESC)`; mas num race astronômico de microsegundo idêntico, `mode="upsert"` aqui sobrescreveria o payload do micro (perderia `volume_delta`/`taker_ratio`/order-flow real). **NÃO trocar para `upsert` sem antes migrar a UNIQUE para incluir `scheduler_group`** (migration N/N+1). Não escreve em `market_metadata`, não puxa order-flow, não computa `_compute_score_fields` (responsabilidade disjunta — esses paths já são alimentados por outros produtores). Não encadeia `score` nem `pipeline_scan` (storms — esses são acionados por `compute_30m` estrutural canônico e `compute_5m` micro respectivamente). Resultado esperado: idade do estrutural lido pelo merged provider cai de ~1770s para ~330s.
...
- **Ordenação determinística de símbolos antes de UPSERT — Tasks #251 + #273 + #310 (consolidado)**: deadlock 40P01 determinístico quando dois workers iteram o mesmo set de símbolos em ordens diferentes (SAVEPOINT por símbolo NÃO libera row-locks — só COMMIT do outer libera). 2026-05-09: **140 deadlocks em 9min** (Task #251). 2026-05-11: regressão nos paths não cobertos (Task #273). 2026-05-20: **segunda regressão pós-#273** no caminho de execução após o ciclo shadow live-L3/continuous shadow/min_alpha_score (Task #310 — `evaluate_signals.py:326` e `execute_buy.py:420` iteravam `merged_by_sym.items()` sem `sorted()`). Fix consolidado: `sorted()` em **todos** os callsites de iteração symbol-wise antes de UPSERT em `market_metadata`/`indicators`/`alpha_scores`/`pipeline_watchlist_assets`/`ohlcv`/`decisions_log`/`trades` — `collect_market_data.py` (5 sites), 3 schedulers, `compute_indicators` (1h/30m/5m), `compute_scores`, `pipeline_scan._upsert_assets`/`_replace_rejection_snapshot`, `ohlcv_backfill_service`, **e desde Task #310 também `evaluate_signals.py` + `execute_buy.py`** (`sorted(merged_by_sym.items())` — worker-execution roda `--concurrency=2` e ordem do dict do `get_merged_indicators` é não-determinística). Lint test `backend/tests/test_pipeline_symbol_ordering_invariants.py` em duas camadas: (1) AST walker que agora também detecta `<dict>.items()/.keys()/.values()` em `_SYMBOL_KEYED_DICT_NAMES` (gap fechado em #310 — antes só pegava `ast.Name`); (2) markers literais por callsite. **NÃO afrouxar os markers do lint** sem provar que a classe de contenção mudou. Bisect quando 40P01 retornar: `backend/docs/runbooks/postgres-deadlock-bisect.md` (Apêndice histórico inclui #310 com a explicação do gap do AST). Timeline 11/05: Apêndice C de `2026-05-08-pipeline-recovery.md`.
...
- **Pipeline XGBoost — label, dedup, NaN (Task #324, 2026-05-21)**: três defeitos compostos deixavam o modelo degenerado silenciosamente. (a) **Label**: `feature_extractor.build_training_dataframe` agora rotula `is_win_fast = 1 if pnl_pct > MIN_WIN_PNL_PCT else 0` (env, default 0.0) e descarta `pnl_pct IS NULL`. Vocabulário canônico de `decisions_log.outcome` é **lowercase `tp`/`sl`** (regime pós-14/05 — timeout descontinuado, trades ficam abertos até TP ou SL); o antigo `outcome == "WIN"` produzia 0% de positivos. (b) **Dedup**: query em `ml_trainer/job.py` usa `DISTINCT ON (symbol, pnl_pct) ORDER BY symbol, pnl_pct, created_at ASC` dentro de subquery + `ORDER BY created_at ASC` na externa — fecha amplificação até 88× por trade único (NEW_SIGNAL/SIGNAL_EVOLVED/visibility-ticks duplicavam o mesmo `pnl_pct` final). (c) **NaN nativo**: `trainer.py` remove `fillna(0.0)` e passa `missing=float("nan")` ao XGBClassifier; `prediction_service.py` usa `features.get(f, nan)` no vetor de inferência. Qualquer produtor novo de feature **DEVE preservar NaN** — 0.0 colapsa "ausente" e "zero real" (ex.: `taker_ratio=0` = 100% venda) na mesma classe semântica. Guardrails extras: trainer aborta com `ValueError` quando `y_train.nunique() < 2` ou `n_pos/n_neg < MIN_SAMPLES_PER_CLASS` (default 30); `job.py` captura essa exceção e faz `sys.exit(0)` (acumulação ≠ falha). `MIN_RECORDS` insuficiente também vira `exit 0` (não 1) — `[TRAINER] insufficient data — skipping run`. Threshold pós-treino calibrado via `precision_recall_curve` (F1-max default, ou `THRESHOLD_CALIBRATION=max_precision_at_recall` com `MIN_RECALL`); valor calibrado vai pro `ml_models.decision_threshold` (substituiu literal `0.500`). MLflow run tags: `label_version=pnl_gt_0_v1`, `dedup_strategy=distinct_symbol_pnl_v1`, `nan_handling=native_xgboost`. **Primeiro run real fica aguardando** acumular ≥`MIN_RECORDS=200` pós-wipe de 17h — disparar Cloud Run Job manualmente para inspeção humana antes de promover.
...
- Pipeline recovery 2026-05-08/09 (4 incidentes correlacionados — `lock_timeout` revertido, recovery TX órfã)
