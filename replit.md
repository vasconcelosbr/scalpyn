...
- **Adding new columns**: write an Alembic migration AND bump the critical column list in `backend/app/main.py::health_check_schema` so production drift is detected proactively. The same list is mirrored in `backend/scripts/check_critical_schema.py` (post-stamp gate, no app imports) — keep both in sync.
