    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[0.92fr_1.08fr]">
        <div className="rounded-2xl border border-white/[0.07] bg-[#11141d] p-5">
          ...
          {analysis && <div className="mt-4 rounded-xl border border-white/[0.07] bg-[#0d1018] p-4"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-[#dce2ee]">{analysis.scope === "REPORT" ? "Análise consolidada" : "Análise do trade"}</span><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${analysis.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-300" : analysis.status === "FAILED" ? "bg-rose-500/10 text-rose-300" : "bg-blue-500/10 text-blue-300"}`}>{analysis.status}</span></div>{analysisBusy && <div className="mt-3 flex items-center gap-2 text-xs text-[#8b95a9]"><Loader2 size={14} className="animate-spin" /> Processando com {analysis.provider}/{analysis.model}</div>}{analysis.error && <p className="mt-3 text-xs text-rose-300">{analysis.error}</p>}{analysis.result?.summary && <p className="mt-3 text-sm leading-6 text-[#b7c0d1]">{analysis.result.summary}</p>}</div>}
        </div>
