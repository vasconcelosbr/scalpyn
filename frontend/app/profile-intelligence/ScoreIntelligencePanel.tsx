...
type Threshold = {
  ...
  lift: Num; win_rate_delta: Num; passed: { trades: number; tp: number; sl: number; timeout: number; win_rate: Num; avg_pnl_pct: Num };
};
type Recommendation = {
...
type SimulationResponse = { simulation?: Threshold & { eliminated_trades: number; baseline: { win_rate: Num; avg_pnl_pct: Num }; passed: Threshold["passed"] & { pnl_sum_pct?: Num; avg_mae_pct?: Num; avg_mfe_pct?: Num; avg_holding_seconds?: Num } } };
...
export default function ScoreIntelligencePanel({ initialData, onCreateManual }: { initialData?: ScoreOverview | null; onCreateManual: (stat: IndicatorEvidence, prefill: ManualAdjustmentPrefill) => void }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="card overflow-hidden"><div className="border-b border-[var(--border-default)] p-4"><h3 className="text-[13px] font-semibold">TP × SL × TIMEOUT</h3><p className="text-[10px] text-[var(--text-tertiary)]">Null permanece ausente; percentis são calculados sobre valores presentes.</p></div><div className="overflow-x-auto"><table className="w-full text-[10px]"><thead><tr className="border-b border-[var(--border-subtle)]">{["Score","TP N","TP Min","TP P25","TP Med","TP Média","TP P75","TP P90","TP Max","SL N","SL Média","TIMEOUT N","TIMEOUT Média","Δ média","Δ mediana","Cobertura"].map(label => <th key={label} className="whitespace-nowrap px-3 py-2 text-left uppercase text-[var(--text-tertiary)]">{label}</th>)}</tr></thead><tbody className="divide-y divide-[var(--border-subtle)]">{stats.map(item => <tr key={item.score}><td className="px-3 py-2"><div className="font-semibold">{SCORE_LABELS[item.score]}</div><div className="font-mono text-[8px] text-[var(--text-tertiary)]">{item.origin}</div></td>{[item.tp.n,item.tp.min,item.tp.p25,item.tp.median,item.tp.mean,item.tp.p75,item.tp.p90,item.tp.max,item.sl.n,item.sl.mean,item.timeout.n,item.timeout.mean,item.delta_mean_tp_sl,item.delta_median_tp_sl].map((value,index) => <td key={index} className="px-3 py-2 font-mono">{typeof value === "number" ? (index === 0 || index === 8 || index === 10 ? value : value.toFixed(2)) : "—"}</td>)}<td className="px-3 py-2">{pct(item.coverage)}</td></tr>)}</tbody></table></div></section>

    <section className="grid gap-4 lg:grid-cols-2"><div className="card p-4"><h3 className="text-[13px] font-semibold">Poder discriminatório</h3><div className="mt-3 space-y-2">{stats.map(item => <div key={item.score} className="grid grid-cols-[1fr_repeat(4,70px)] gap-2 rounded border border-[var(--border-subtle)] p-2 text-[10px]"><span>{SCORE_LABELS[item.score]}</span><span title="TP mean - SL mean">Δ {num(item.delta_mean_tp_sl)}</span><span title="Área sob a curva individual; maior score tratado como TP">AUC {num(item.auc,3)}</span><span title="Kolmogorov-Smirnov">KS {num(item.ks_statistic,3)}</span><span>{item.confidence}</span></div>)}</div></div><div className="card p-4"><h3 className="text-[13px] font-semibold">Thresholds atuais</h3><p className="mt-1 text-[10px] text-[var(--text-tertiary)]">Origem: versão exata do Score Engine.</p><div className="mt-3 space-y-2">{(data.current_thresholds || []).map(item => <div key={item.name} className="rounded border border-[var(--border-subtle)] p-3 text-[10px]"><div className="flex justify-between"><span className="font-semibold">Score {item.name} ≥ {item.threshold}</span><span>{item.passed.trades} aprovados</span></div><div className="mt-1 grid grid-cols-4 gap-2 text-[var(--text-secondary)]"><span>TP {item.passed.tp}</span><span>SL {item.passed.sl}</span><span>TIMEOUT {item.passed.timeout}</span><span>WR {pct(item.passed.win_rate)}</span></div><div className="mt-1">Lift {num(item.lift)} · redução {pct(item.volume_reduction)} · P&L {num(item.passed.avg_pnl_pct)}%</div></div>)}{!data.current_thresholds?.length && <div className="text-[11px] text-[var(--text-tertiary)]">Nenhum threshold numérico persistido para este score engine.</div>}</div></div></section>
