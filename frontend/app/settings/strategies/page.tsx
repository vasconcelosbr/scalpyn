...
import {
  ...
  MTFRuntimeAudit, StrategySettingsBundle, StrategySettingsValidation,
...
const COPY: Record<string, { label: string; help?: string }> = {
  "spot_engine.scanner.multilayer_contract.enabled": { label: "Observação multicamada", help: "Quando ligada, registra o MTF em Shadow. operational_effect=false impede qualquer autoridade sobre ordens reais." },
  "spot_engine.scanner.multilayer_contract.activation_mode": { label: "Modo MTF", help: "Esta release aceita somente DRAFT ou SHADOW." },
  "spot_engine.scanner.multilayer_contract.operational_effect": { label: "Efeito sobre ordens", help: "Permanece falso nesta release." },
  ...
  "ml_shadow.ml_l3_managed_exit.fee_roundtrip_pct": { label: "Taxas totais do contrato ML L3 (%)" },
  "ml_shadow.ml_l3_managed_exit.slippage_roundtrip_pct": { label: "Slippage total do contrato ML L3 (%)", help: "Custo adicional de entrada e saída. Alterar este valor inicia uma população de treinamento diferente." },
  "ml_shadow.ml_l3_managed_exit.max_holding_seconds": { label: "Horizonte máximo do alvo ML L3 (segundos)", help: "Resultados além deste prazo são excluídos do treino; a posição continua seguindo a política de saída." },
...
export default function StrategySettingsPage() {
  ...
  const [mtfAudit, setMtfAudit] = useState<MTFRuntimeAudit | null>(null);

  const refresh = async () => { setBusy(true); setError(null); try { const result = await loadStrategySettings(); setSaved(result.config); setDraft(editablePayload(result.config)); setCatalog(result.catalog); setPersisted(result.persisted); try { setMtfAudit(await loadMTFRuntimeAudit()); } catch { setMtfAudit(null); } } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(false); } };
  ...
  const mtfContract = mtfAudit?.multilayer_contract;
  const mtfOperational = mtfContract?.operational_effect === true;
  const mtfCalibration = mtfAudit?.latest_calibration;
  const mtfStatisticalGate = mtfAudit?.statistical_gate;
  const mtfWaived = mtfStatisticalGate?.status === "WAIVED_FOR_SHADOW";
  const mtfAcceptedRows = Number(mtfCalibration?.dataset_manifest?.accepted_rows ?? 0);
  const mtfRequestedRows = Number(mtfCalibration?.dataset_manifest?.requested_rows ?? 0);
  const mtfCoverage = mtfAudit?.runtime.coverage ?? [];
  const mtfTechnicalFunctional = mtfAudit?.runtime.technical_status === "SHADOW_FUNCTIONAL";
  const mtfStatisticallyApproved = mtfAudit?.runtime.statistical_status === "APPROVED";
  const mtfIdentityHealth = mtfAudit?.runtime.identity_health ?? {};
  ...
  return <div className="space-y-6 pb-12">
    ...
    <div className="grid gap-4 lg:grid-cols-4"><div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-secondary)] p-5"><WalletCards className="h-5 w-5 text-emerald-400" /><h2 className="mt-3 font-semibold">Spot real</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">Entrada, capital, venda, underwater, DCA e filtro macro.</p></div><div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-secondary)] p-5"><Boxes className="h-5 w-5 text-blue-400" /><h2 className="mt-3 font-semibold">Shadow</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">Barreiras, custos, timeout, TTT, valor e contratos técnicos.</p></div><div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-secondary)] p-5"><ShieldCheck className="h-5 w-5 text-violet-400" /><h2 className="mt-3 font-semibold">Snapshot imutável</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">Trades abertos não são recalculados quando esta tela muda.</p></div><div className="rounded-2xl border border-blue-500/25 bg-[var(--bg-secondary)] p-5"><Layers3 className="h-5 w-5 text-blue-400" /><div className="mt-3 flex flex-wrap items-center gap-2"><h2 className="font-semibold">MTF 1h → 15m → 5m</h2><EffectBadge tone={mtfTechnicalFunctional ? "green" : "amber"}>{mtfTechnicalFunctional ? "SHADOW FUNCIONAL" : "SHADOW DEGRADADO"}</EffectBadge>{!mtfStatisticallyApproved && <EffectBadge tone="amber">ESTATISTICAMENTE NÃO APROVADO</EffectBadge>}</div><p className={`mt-1 text-xs ${mtfOperational ? "text-red-300" : "text-emerald-300"}`}>{mtfOperational ? "Efeito operacional indevido" : "Sem efeito sobre ordens reais"}</p><p className="mt-2 text-[11px] text-[var(--text-tertiary)]">Contextos completos desde o contrato atual: {mtfAudit?.runtime.v5_complete_contexts_current_contract ?? "indisponível"} · replay controlado: {mtfAudit?.runtime.controlled_replay?.status ?? "indisponível"}</p></div></div>
    <section className="rounded-2xl border border-blue-500/25 bg-[var(--bg-secondary)] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Aceite MTF observacional</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">Cobertura dos produtores, calibração vinculada e estado persistente dos setups.</p></div><div className="flex flex-wrap gap-2"><EffectBadge tone={mtfTechnicalFunctional ? "green" : "amber"}>{mtfTechnicalFunctional ? "SHADOW FUNCIONAL" : "SHADOW DEGRADADO"}</EffectBadge><EffectBadge tone={mtfStatisticallyApproved ? "green" : "amber"}>{mtfStatisticallyApproved ? "ESTATISTICAMENTE APROVADO" : "ESTATISTICAMENTE NÃO APROVADO"}</EffectBadge></div></div>{mtfWaived && <div className="mt-4 flex gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-300" /><div className="text-xs leading-relaxed text-amber-100"><strong>Ativação observacional autorizada por waiver humano.</strong> A calibração não passou por amostra insuficiente; os thresholds permanecem não validados e o contexto MTF não pode autorizar ordens. <span className="text-amber-200">Escopo: {mtfStatisticalGate?.authorization_scope ?? "OBSERVATIONAL_ONLY"} · amostra aceita: {mtfAcceptedRows}/{mtfRequestedRows || "indisponível"}.</span></div></div>}<div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{mtfCoverage.map((item) => { const health = mtfIdentityHealth[`${item.timeframe}:${item.scheduler_group}`]; return <div key={`${item.timeframe}-${item.scheduler_group}`} className="rounded-xl border border-[var(--border-subtle)] bg-black/10 p-3"><div className="flex items-center justify-between gap-2"><p className="text-[10px] uppercase text-[var(--text-tertiary)]">{item.timeframe} · {item.scheduler_group}</p><EffectBadge tone={health?.coverage_complete && health?.fresh ? "green" : "amber"}>{health?.coverage_complete && health?.fresh ? "SAUDÁVEL" : "DEGRADADO"}</EffectBadge></div><p className="mt-1 text-sm font-semibold">{item.symbols} símbolos</p><p className="mt-1 truncate text-[10px] text-[var(--text-tertiary)]">{item.latest ?? "sem snapshot"}</p></div>; })}</div><div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-[var(--border-subtle)] p-3"><p className="text-[10px] uppercase text-[var(--text-tertiary)]">Execução de calibração</p><p className="mt-1 text-xs">{mtfCalibration ? `${mtfCalibration.policy_version} · ${mtfCalibration.id}` : "Nenhuma execução aprovada"}</p>{mtfCalibration?.failure_reason && <p className="mt-2 text-xs text-amber-300">Bloqueio: {mtfCalibration.failure_reason}</p>}</div><div className="rounded-xl border border-[var(--border-subtle)] p-3"><p className="text-[10px] uppercase text-[var(--text-tertiary)]">Máquina de estados L2</p><p className="mt-1 text-xs">{mtfAudit?.runtime.l2_setup_states.length ? mtfAudit.runtime.l2_setup_states.map((item) => `${item.state}: ${item.symbols}`).join(" · ") : "Sem estados persistidos"}</p></div></div></section>
