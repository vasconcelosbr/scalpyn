...
function RadarProviderCard() {
  const [status, setStatus] = useState<RadarStatus>({
    is_configured: false, key_hint: null,
    test_status: null, test_error: null, last_tested_at: null,
  });
  const [editing, setEditing] = useState(false);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const radar = data.find((p: { provider: string }) => p.provider === "radar");
        if (radar) {
          setStatus({
            is_configured: radar.is_configured,
            key_hint: radar.key_hint,
            test_status: radar.test_status,
            test_error: radar.test_error,
            last_tested_at: radar.last_tested_at,
          });
          setEditing(!radar.is_configured);
        }
      }
    } catch {}
    setLoading(false);
  ...
  const handleSave = async () => {
    ...
    setSaving(true); setError(null); setTestResult(null);
    try {
      const res = await fetch("/api/ai-keys/radar", {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ api_key: apiKey.trim() }),
      });
  ...
  const handleTest = async () => {
    setTesting(true); setTestResult(null);
    try {
      const res = await fetch("/api/ai-keys/radar/test", { method: "POST", headers: authHeaders() });
      const data = await res.json();
      setTestResult({ success: data.success, message: data.message });
      if (data.success) load();
    } catch { setTestResult({ success: false, message: "Erro de conexão." }); }
  ...
  const handleDelete = async () => {
    if (!confirm("Remover a chave Market Catalyst Radar?")) return;
    await fetch("/api/ai-keys/radar", { method: "DELETE", headers: authHeaders() });
    load();
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "16px 20px", borderBottom: (editing || status.is_configured) ? "1px solid var(--border-subtle)" : "none" }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>Market Catalyst Radar</span>
            {status.is_configured && isOk && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, padding: "3px 8px", borderRadius: 20, fontWeight: 600, background: "var(--color-profit-muted)", color: "var(--color-profit)" }}>
                <CheckCircle2 size={10} />Conectado
              </span>
          ...
          <p style={{ fontSize: 12, color: "var(--text-secondary)", margin: 0, lineHeight: 1.5 }}>
            <strong style={{ color: "var(--text-primary)" }}>Descoberta dinâmica de ativos.</strong> Alimenta o toggle "Market Catalyst Radar" em Strategy Pools — sincroniza os top ativos por pressão compradora (mdatahub) a cada 10 minutos.
          </p>
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            {["Gate.io spot", "Buy-pressure", "10min"].map(tag => (
              <span key={tag} style={{ fontSize: 10, padding: "2px 7px", borderRadius: 20, background: "rgba(255,255,255,0.04)", border: "1px solid var(--border-subtle)", color: "var(--text-tertiary)" }}>{tag}</span>
            ))}
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
          <a href="https://mdatahub.scalpyn.com/radar" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--text-tertiary)", textDecoration: "none" }}>
            <ExternalLink size={12} />Docs
          </a>
          {status.is_configured && !editing && (
            <>
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              <input
                ...
                value={apiKey}
                onChange={e => setApiKey(e.target.value)}
                placeholder="Cole sua Radar API key aqui (rdr_...)..."
                autoComplete="off"
                style={{ ...inputStyle, borderColor: error ? "var(--color-loss-border)" : "var(--border-default)" }}
              />
              <button onClick={() => setShowKey(!showKey)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}>
            ...
            <p style={{ fontSize: 11, color: "var(--text-tertiary)", margin: "6px 0 0" }}>
              Chave do Market Catalyst Radar (mdatahub), formato <code>rdr_...</code>.
            </p>
          </div>
          {error && (
            <div style={{ fontSize: 12, padding: "8px 12px", borderRadius: 7, background: "var(--color-loss-muted)", border: "1px solid var(--color-loss-border)", color: "var(--color-loss)" }}>{error}</div>
