...
OUT = r"C:\Users\ricar\AppData\Local\Temp\claude\C--WINDOWS-system32\8739466e-e897-4b8b-a08a-d8c741a5df75\scratchpad\etapa3_grid_results.json"
...
def build_policies():
    policies = {}
    for activation in (0.6, 0.8, 1.0, 1.5):
        for trail in (0.25, 0.35, 0.50, 0.75, 1.00):
            name = f"P1_FIXED_act{activation}_trail{trail}"
            policies[name] = {
                "family": "FIXED",
                "params": {"activation_profit_pct": activation, "hwm_trail_pct": trail},
                "fn": (lambda hwm, ep, a=activation, t=trail: floor_fixed(hwm, ep, a, t)),
            }
    for k in (0.20, 0.30, 0.40, 0.50):
        name = f"P3_PROPORTIONAL_k{k}"
        policies[name] = {
            "family": "PROPORTIONAL",
            "params": {"k": k},
    ...
    stepped_variants = {
        "P2_BASE_LADDER_NO_TRAIL_BELOW": {"steps": [(2.0, 1.5), (4.0, 2.5)], "base": None},
        "P2_BASE_LADDER_WIDE_FIXED_BELOW": {"steps": [(2.0, 1.5), (4.0, 2.5)], "base": (1.0, 0.75)},
        "P2_WIDER_FIRST_STEP": {"steps": [(1.5, 0.8), (3.0, 2.0), (5.0, 3.5)], "base": None},
        "P2_EARLY_STEP_NEAR_ACTIVATION": {"steps": [(1.2, 0.5), (2.5, 1.8), (4.5, 3.0)], "base": None},
    }
    for name, cfg in stepped_variants.items():
        policies[name] = {
            "family": "STEPPED",
            "params": cfg,
            "fn": (lambda hwm, ep, c=cfg: floor_stepped(hwm, ep, c["steps"], c["base"])),
        }
    return policies
...
def main():
    ...
    per_trade_cache = {}  # trade id -> (entry_price, window, tp, sl, timeout, never_sell, protected_pct, entry_at, recorded_outcome, recorded_net)
    for row in rows:
        sym = row["symbol"]
    ...
    for pname, pdef in policies.items():
        ...
        for row in rows:
            ...
            if exit_nom is None or outcome is None:
                gross_pct = None
            else:
                gross_pct = (exit_nom / t["entry_price"] - 1) * 100
        ...
        cell_results[pname] = {
            "family": policies[pname]["family"],
            "params": policies[pname]["params"] if policies[pname]["family"] != "STEPPED" else {
                "steps": policies[pname]["params"]["steps"], "base": policies[pname]["params"]["base"]
            },
            "n": n,
            "outcome_composition": outcomes_ct,
