...
# Symptom: a worker container killed mid-transaction (SIGTERM/OOM/network blip
# from Cloud Run) leaves an asyncpg connection without an explicit ROLLBACK.
# The Postgres backend then sits ``idle in transaction`` forever (we observed
# PID 601816 holding RELEASE SAVEPOINT for 11 min), pinning row-locks on
# ``market_metadata``. Subsequent collect_5m / collect_all runs queue on those
# locks and time out at command_timeout=180 s → bola de neve.
#
...
#
# 2026-05-09 REGRESSION RECOVERY — ``lock_timeout=30000`` was tried in the same
# 2026-05-08 deploy and BACKFIRED within 12 h. Symptom (full incident in
# ``backend/docs/runbooks/2026-05-08-pipeline-recovery.md``): hot symbols
# (ETH_USDT especially, also GT_USDT, FLOKI_USDT, NEXO_USDT) chronically
# failing every collect cycle with ``asyncpg.exceptions.LockNotAvailableError:
# canceling statement due to lock timeout``, ``pipeline_scan.scan`` raising
# ``Fatal error`` and re-queueing on the execution queue (backlog 2446 in
# 14 h). Root: 30 s is shorter than legitimate contention between 5 Cloud Run
# workers + Gate.io WS UPSERTing into the same hot rows. ``command_timeout=180s``
