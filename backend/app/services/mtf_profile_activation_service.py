...
def _profile_document_to_config(document: Mapping[str, Any], layer: str) -> dict[str, Any]:
...
def parse_activation_document(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("import_mode") != IMPORT_MODE:
        raise ValueError(f"import_mode must be {IMPORT_MODE}")
    ...
    for layer in ("L1", "L2"):
        ...
        parsed_profiles[layer] = {
            ...
            "config": _profile_document_to_config(item, layer),
        }
    ...
    if not isinstance(thresholds_hashes, Mapping) or set(thresholds_hashes) != {"L1", "L2"}:
    ...
    parsed_calibration = {
        ...
        "thresholds_hashes": {
            layer: _hash(thresholds_hashes[layer], f"calibration.thresholds_hashes.{layer}")
            for layer in ("L1", "L2")
        },
    }
...
async def activate_existing_mtf_profiles(
    ...
    for layer in ("L1", "L2"):
        ...
        activations[layer] = await activate_profile_config(
            ...
            change_source="mtf_governed_activation",
            change_description=f"[{IMPORT_MODE}] update existing {layer}",
            require_feature_identity=True,
            version_idempotency_namespace=(
                f"mtf-activation:{parsed['calibration']['run_id']}"
            ),
        )
    ...
    """), {
        ...
        "applied_by": str(user_id), "import_mode": IMPORT_MODE,
        "document_hash": canonical_hash(payload),
