...
Three metrics exposed on ``/metrics`` — names align with the Grafana
dashboard contract requested in the Task #234 hotfix brief:

  * ``ohlcv_ingest_received_total{symbol,timeframe}`` — Counter incremented
    once per fetch_ohlcv response. Labels are bounded by the active pool
    (currently BTC-only) so cardinality stays small.
  * ``ohlcv_ingest_persisted_total{symbol,timeframe}`` — Counter
    incremented per successful UPSERT batch (one per symbol per cycle).
  * ``ohlcv_staleness_seconds{symbol,timeframe}`` — Gauge of the age
    (now - latest_candle_time) immediately after persistence; primary
    signal for the ``ingestion_stale`` alert.

Operators alert on:
  * ``rate(ohlcv_ingest_received_total[10m]) == 0`` for any active symbol.
  * ``ohlcv_staleness_seconds > 1800`` for any active symbol.
...
def record_latest_age(symbol: str, timeframe: str, age_seconds: float) -> None:
    if _OHLCV_STALENESS is None:
        return
    try:
        _OHLCV_STALENESS.labels(symbol=symbol, timeframe=timeframe).set(
            max(0.0, float(age_seconds))
        )
    except Exception as exc:  # pragma: no cover
        logger.debug("[ohlcv_metrics] staleness set failed: %s", exc)
