...
class ShadowTradeDetail(ShadowTradeRead):
    ...
    entry_risk_features: Optional[Dict[str, Any]] = None
    entry_risk_capture_status: str = "NOT_AVAILABLE"
    entry_risk_captured_at: Optional[datetime] = None
...
class ShadowTradeSummary(BaseModel):
    ...
    positive: int  # finalizados com COALESCE(net_return_pct, pnl_pct) > 0
    measured: int  # finalizados com retorno persistido disponível
    win_rate: float  # positive / measured × 100
...
class ProfileReportRow(BaseModel):
    """Canonical profile/watchlist performance ranking row."""
    ...
    avg_pnl_pct: Optional[float] = None
    avg_holding_win_seconds: Optional[float] = None
    tp_4h_count: int = 0
    tp_4h_wins: int = 0
    tp_4h_rate: Optional[float] = None
    ev_score: float
    stat_confidence: str
