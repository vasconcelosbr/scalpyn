...
def test_build_indicators_snapshot_emits_expected_shape():
    rows = [
        (
            "structural",
            NOW - timedelta(minutes=10),
    ...
    # Every required-core key is present in the snapshot
    for key in REQUIRED_CORE_INDICATORS:
        assert key in snap, f"snapshot must always carry {key}"
    ...
    taker_entry = snap["taker_ratio"]
    assert taker_entry["value"] == 0.51
    assert taker_entry["source_group"] == "microstructure"
