"""Make array_guards[].identity's required shape (object, never a bare
scalar) explicit in the system prompt (FIX-AC-OBS-004_r2 second follow-up).

Root cause, isolated by direct reproduction of governed_change_service.
create_dry_run() with the model's exact real proposal (no API call needed --
this is pure backend validation): the model correctly resolved the concrete
path (/entry_triggers/conditions/4/min) and cited real evidence, but set
change.array_guards_json to [{"path":"...","identity":"rsi"}] -- identity as
a bare string. _guard_matches() (governed_change_service.py) compares a
scalar identity with `item == identity`, which a dict array element can
never satisfy, so _validate_array_guards() rejects it as "missing,
ambiguous, or at another index" -- surfaced to the user as the generic
GOVERNED_CHANGE_REQUIRES_AN_UNAMBIGUOUS_TYPED_TARGET / "proposed paths do
not match the current configuration contract" message from
draft_proposal_if_confirmed's broad except clause.

Re-running the identical proposal with identity as an object
({"indicator": "rsi"}, matching the actual key present on the persisted
array element) passed create_dry_run() cleanly and produced a real
DRY_RUN plan. This confirms the governed-write pipeline is fully functional
end to end (168-171 already fixed the systemic failures); the model just
needs the identity-shape instruction spelled out unambiguously -- the prior
wording ("identity copies stable persisted identity such as id, rule_id,
field, indicator, name, or left+right") reads as "copy the value", not
"wrap the key and value in an object".

This is a system_template text-only change -- no output_schema_json change,
so unlike 171 it carries no risk of an Anthropic schema-keyword rejection
and was not re-verified against the live API before deploying.

Revision ID: 172_chat_identity_object
Revises: 171_chat_no_if_then_else
...
revision = "172_chat_identity_object"
down_revision = "171_chat_no_if_then_else"
branch_labels = None
depends_on = None
...
def _prompt_content() -> dict[str, object]:
            ...
            **_shared_change_properties,
            ...
            "array_guards_json": {
                ...
                "description": (
                    ...
                    "array element traversed by path; identity is always a JSON object "
                    "mapping one stable key (id, rule_id, field, indicator, name, or "
                    "left+right) to its exact persisted value, e.g. {\"indicator\":\"rsi\"} "
                    "-- never a bare string or scalar; encode [] when no array is traversed."
                ),
            },
        },
    }
    ...
    return {
        ...
        "system_template": (
            ...
            "have one array_guards_json entry whose path ends at that index and whose identity is a "
            "JSON OBJECT mapping one stable persisted key -- id, rule_id, field, indicator, name, or "
            "left+right -- to its exact value copied from that array element, for example "
            "{\"indicator\":\"rsi\"} or {\"id\":\"abc123\"}; identity is NEVER a bare string, number, "
            "or the value alone -- it must always be a {\"key\":\"value\"} object using the exact key "
            "name present on that element. "
...
def upgrade() -> None:
    ...
    prompt_id = str(uuid.uuid5(
        PROMPT_NAMESPACE,
        "analysis-chat-governed-change@1.10.0",
    ))
    ...
    if (
        ...
        raise RuntimeError("ANALYSIS_CHAT_PROMPT_1_10_CONFLICT")
