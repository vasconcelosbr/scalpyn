...
def upgrade() -> None:
    ...
    """))
    # No speculative historical backfill. NULL is interpreted as
    # LEGACY_UNRESOLVED by readers and remains ineligible by default.
    ...
    # Preserve v77 and its artifact; only correct its governance classification.
    op.execute(sa.text("""
        UPDATE ml_models
    ...
    # Safe rollout defaults. Existing explicit values are preserved.
    op.execute(sa.text("""
