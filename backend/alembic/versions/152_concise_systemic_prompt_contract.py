"""Seed the concise systemic provider output contract.

Revision ID: 152_concise_systemic_prompt
Revises: 151_systemic_prompt_schema
"""
...
revision = "152_concise_systemic_prompt"
down_revision = "151_systemic_prompt_schema"
branch_labels = None
depends_on = None

EXPECTED_PROMPT_HASH = "9e98c817db179a0347c488bbac15d350123e0225b69f010dbde0cefcff80c3a8"
...
def _prompt_values() -> dict:
    ...
    if prompt.content_hash != EXPECTED_PROMPT_HASH:
        raise RuntimeError("SYSTEMIC_MULTIMODULE_PROMPT_2_0_2_HASH_DRIFT")
          ...
          IF EXISTS (
            SELECT 1 FROM ai_prompt_versions
             WHERE prompt_key = 'systemic-multimodule'
               AND semantic_version = '2.0.2'
               AND content_hash <> '{EXPECTED_PROMPT_HASH}'
          ) THEN
            RAISE EXCEPTION 'SYSTEMIC_MULTIMODULE_PROMPT_2_0_2_DATABASE_CONFLICT';
          END IF;
        END $$;
    """))
    op.execute(sa.text(f"""
