"""Enable governed bulk profile configuration proposals.

Revision ID: 165_chat_bulk_profile_config
Revises: 164_chat_bulk_profile_actions
...
revision = "165_chat_bulk_profile_config"
down_revision = "164_chat_bulk_profile_actions"
branch_labels = None
depends_on = None

PROMPT_NAMESPACE = uuid.UUID("1f9da597-c633-41f3-af51-8a77462a9b21")
...
def _prompt_content() -> dict[str, object]:
    ...
    proposal = {
        ...
        "properties": {
            ...
            "changes": {
                "type": "array", "minItems": 1, "maxItems": 100, "items": change,
            },
        },
    }
    output = {
    ...
    return {
        ...
        "system_template": (
            ...
            "UPDATE_PROFILE_CONFIG for fields inside one profile, UPDATE_PROFILE_CONFIG_SET for "
            "fields across multiple existing profiles, UPDATE_CONFIG_PROFILE for one configuration "
            "family, and SET_PROFILE_ACTIVE_STATUS to activate or deactivate profiles without "
            "deleting them. For either multi-profile operation, put every owned profile UUID in "
            "target.profile_ids and set matching profile_id and profile_name on every change. Keep "
            "paths relative to each profile configuration. Use only actual field paths and values "
            "supported by the supplied evidence; do not translate narrative recommendations into "
            "invented schema fields. Keep the response compact. Every change must cite supplied "
            "evidence IDs. Never invent IDs, values, or evidence. Orders, credentials, secrets, "
            "arbitrary SQL/code, ML promotion, deletion, and runtime-gate self-modification are "
            "outside this contract. Return JSON matching the schema."
        ),
        "user_template": (
...
def upgrade() -> None:
    ...
    """), {
        "id": str(uuid.uuid5(PROMPT_NAMESPACE, "analysis-chat-governed-change@1.2.0")),
        **prompt,
