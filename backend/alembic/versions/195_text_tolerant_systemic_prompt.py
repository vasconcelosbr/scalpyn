"""Make systemic text length advisory while preserving structural guards.

Revision ID: 195_text_tolerant_prompt
Revises: 194_systemic_prompt_contract
"""
...
revision = "195_text_tolerant_prompt"
down_revision = "194_systemic_prompt_contract"
branch_labels = None
depends_on = None

EXPECTED_PROMPT_HASH = "215398fa82c7257f8516a0394ce77ed735ba34809fc121e065760fe4c0befc9e"
...
def _prompt_values() -> dict:
    ...
    if prompt.content_hash != EXPECTED_PROMPT_HASH:
        raise RuntimeError("SYSTEMIC_MULTIMODULE_PROMPT_2_0_7_HASH_DRIFT")
    seeded_at = datetime(2026, 8, 21, 15, 45, tzinfo=timezone.utc)
    return {
        "id": prompt.id,
...
def upgrade() -> None:
        ...
        sa.text("""
            SELECT content_hash
             ...
             WHERE prompt_key = 'systemic-multimodule'
               AND semantic_version = '2.0.7'
        """)
    ).scalar_one_or_none()
    if conflict is not None and conflict != EXPECTED_PROMPT_HASH:
        raise RuntimeError("SYSTEMIC_MULTIMODULE_PROMPT_2_0_7_DATABASE_CONFLICT")
    bind.execute(
        sa.text("""
    ...
    if persisted != EXPECTED_PROMPT_HASH:
        raise RuntimeError("SYSTEMIC_MULTIMODULE_PROMPT_2_0_7_PERSISTENCE_FAILED")
        ...
        sa.text("""
            SELECT count(*)
             ...
             WHERE p.prompt_key = 'systemic-multimodule'
               AND p.semantic_version = '2.0.7'
        """)
    ).scalar_one()
    if references:
        raise RuntimeError("SYSTEMIC_MULTIMODULE_PROMPT_2_0_7_IN_USE")
    bind.execute(
        sa.text("""
