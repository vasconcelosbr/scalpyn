"""Let the model target a scoring rule by its persisted id instead of a
free-typed JSON pointer (FIX-AC-GOV-002 Fase 5).

The one real "wrong schema" production failure
(EXPECTED_AN_OBJECT_AT_/SCORING/RULES/RSI_OVERBOUGHT_PENALTY) was the model
inventing a path -- /scoring/rules/rsi_overbought_penalty -- that matches
nothing in the real schema: profile scoring.rules is an empty array (rules
live in the global score document only), the real key is scoring_rules (no
"scoring." prefix), and the real rule id is a slug that encodes the exact
condition (e.g. rule_rsi_between_68_78), never a vague name like
"rsi_overbought_penalty".

Prompt v1.5.0 already named and forbade this exact path in prose ("Paths
such as /scoring/rules/rsi_overbought_penalty ... are invalid") and the
model produced it anyway a few hours later in production. Reinforcing the
prose further was tried and failed; this migration removes the need for
the model to construct the pointer at all for the one concrete case
observed: when editing a scoring rule's points, the model supplies only
change.rule_id (the exact persisted scoring_rules[].id) and value_json;
the backend (see _translate_score_rule_points in
analysis_chat_handler.py) resolves the real array index, old value and
identity guard deterministically. The general path-based contract is
unchanged for every other case.

Revision ID: 169_chat_score_rule_id
Revises: 168_chat_evidence_labels
...
revision = "169_chat_score_rule_id"
down_revision = "168_chat_evidence_labels"
branch_labels = None
depends_on = None
...
def _prompt_content() -> dict[str, object]:
    ...
    return {
        ...
        "system_template": (
            ...
            "proposal=null; never guess, coerce, skip profiles, or invent schema. For identical "
...
def upgrade() -> None:
    ...
    prompt_id = str(uuid.uuid5(
        PROMPT_NAMESPACE,
        "analysis-chat-governed-change@1.7.0",
    ))
    ...
    if (
        ...
        raise RuntimeError("ANALYSIS_CHAT_PROMPT_1_7_CONFLICT")
